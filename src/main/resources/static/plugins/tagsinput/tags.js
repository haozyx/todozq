...
$.fn.addTags = function (args) {
	if(args){
	    var data= args.split(",");
	    $.each(data,function(i,v){
	    	if(v){
	    		let template="<span class=\"tag\"><span class=\"text\" _value='"+v+"'>"+v+"</span><span class=\"close\">&times;</span></span>\n";
		    	 $('#myTags .data').append(template);
	    	}	
	    });
	}
};
...
function _add_input_tag(el,data,text){
    let template="<span class=\"tag\"><span class=\"text\" _value='"+text+"'>"+text+"</span><span class=\"close\">&times;</span></span>\n";
